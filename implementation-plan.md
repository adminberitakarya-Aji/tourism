...
| 0.2 | Scaffold `apps/web` | Next.js + TypeScript init, jalan di `pnpm dev`, halaman default render | 0.1 | Not Started |
| 0.3 | Scaffold `services/api` | NestJS + TypeScript init, jalan di `pnpm start:dev`, endpoint health-check (`GET /health`) return 200 | 0.1 | Not Started |
| 0.4 | Scaffold `packages/core` | Package kosong dengan `index.ts`, ter-resolve sebagai dependency dari `apps/web` & `services/api` via workspace | 0.1 | Not Started |
